...
const DICTIONARY: Record<string, TooltipDefinition> = {
  ...
  over: {
    title: "OVER clause",
    description:
      "Defines the window partitioning and ordering for window analytical functions.",
  ...
  "partition by": {
    ...
    description:
      "Divides query result rows into groups for independent window function calculation.",
  ...
  row_number: {
    ...
    description:
      "Assigns a unique, consecutive integer to each row starting at 1 within each partition.",
  ...
  dense_rank: {
    ...
    description:
      "Assigns consecutive rank numbers without gaps for ties (e.g. 1, 2, 2, 3).",
    order: "Window calculation",
  ...
  lag: {
    title: "LAG() function",
    description:
      "Accesses data from a previous row in the partition without a self-join.",
    order: "Window calculation",
  ...
  lead: {
    title: "LEAD() function",
    description:
      "Accesses data from a subsequent row in the partition without a self-join.",
    order: "Window calculation",
  ...
  case: {
    ...
    description:
      "Conditional expression that returns values based on evaluated boolean criteria.",
    order: "Expression evaluation",
  ...
  cte: {
    ...
    description:
      "Defines a named temporary result set that exists only within query scope.",
  ...
  select: {
    title: "SELECT clause",
    description:
      "Specifies which columns or computed fields the query should return.",
  ...
  from: {
    ...
    description:
      "Identifies the source tables and matches tables using JOIN criteria.",
    order: "Evaluated 1st",
  ...
  where: {
    title: "WHERE clause",
    description:
      "Filters individual rows based on a boolean condition before grouping.",
    order: "Evaluated 2nd",
  ...
  having: {
    title: "HAVING clause",
    description:
      "Filters grouped rows and aggregates. Evaluated after GROUP BY.",
    order: "Evaluated 4th",
  ...
  "order by": {
    ...
    description:
      "Sorts the final result set by one or more columns (ASC or DESC).",
    order: "Evaluated 6th",
  ...
  limit: {
    title: "LIMIT clause",
    description:
      "Restricts the maximum number of rows returned in the final output.",
  ...
  join: {
    title: "JOIN clause",
    description:
      "Combines records from multiple tables based on matching values.",
  ...
  with: {
    ...
    description:
      "Defines a temporary named result set to simplify complex queries.",
...
export default function SqlSyntaxTooltip({
  keyword,
  children,
}: SqlSyntaxTooltipProps) {

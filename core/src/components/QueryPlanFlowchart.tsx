      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <strong style={{ fontSize: "12px" }}>
            Visual Optimizer Pipeline (Cost & Row Estimates)
          </strong>
        </div>
        <div
          style={{
        ...
        >
          <button
            onClick={() => setFlowchartZoom((z) => Math.max(0.5, z - 0.1))}
            style={{
              background: "none",
              border: "none",
              cursor: "pointer",
          ...
          <button
            onClick={() => setFlowchartZoom((z) => Math.min(2, z + 0.1))}
            style={{
              background: "none",
              border: "none",
              cursor: "pointer",
      ...
      >
        ...
        >
          ...
          {layout.nodes.map((node) => {
            ...
            const isSort =
              upDetail.includes("SORT") || upDetail.includes("B-TREE");
            ...
            return (
              <g
                key={`node-${node.id}`}
                className="plan-node"
                transform={`translate(${node.x - 95}, ${node.y - 28})`}
              >
                <rect
                  width="190"
                  height="56"
                ...
                >
                  {node.detail.length > 27
                    ? `${node.detail.substring(0, 25)}...`
                    : node.detail}

// Extracted metadata from curriculum
...
export const realLessons: Record<number, Lesson> = {
  1: {
...
LIMIT 10;`,
        ...
        explanation:
          "SELECT * means 'give me all columns'. FROM customers tells SQL which table. " +
...
LIMIT 10;`,
        ...
        explanation:
          "customer_id here is a FOREIGN KEY — it matches customer_id in the customers " +
...
LIMIT 20;`,
        ...
        explanation:
          "AND means both conditions must be true. Only rows that are Delivered AND have " +
...
ORDER BY city;`,
        ...
        explanation:
          "IN is cleaner than writing city = 'Mumbai' OR city = 'Bengaluru' OR ... Use IN " +
...
Sorting NULL values:
  ...
  MySQL (used here): NULLs sort first in ASC
...
LIMIT 10;`,
        ...
        explanation:
          "We filter to delivered orders, compute net_amount in SELECT, then sort by that " +
...
ORDER BY region ASC, signup_date DESC;`,
        ...
        explanation:
          "First sort by region alphabetically. Within the same region, show the most " +
...
LIMIT 5;`,
        ...
        explanation:
          "ORDER BY total_amount DESC ensures the highest values are first. LIMIT 5 then " +
...
LIMIT 10 OFFSET 10;`,
        ...
        explanation:
          "Skip the first 10 rows (page 1) and return the next 10 (page 2). Used in any " +
...
Handling NULLs:
  ...
  IFNULL(a, b)  → return b if a is NULL (MySQL/MySQL)
...
WHERE discount_amount IS NULL
   OR discount_amount = 0;`,
        ...
        explanation:
          "IS NULL finds missing values. COALESCE(discount_amount, 0) replaces NULL with 0 " +
...
WHERE status = 'Delivered';`,
        ...
        explanation:
          "COALESCE(discount_amount, 0) makes the AVG denominator include all orders, not " +
...
Most important functions (work in MySQL + most databases):
  ...
  CONCAT(col1, ' ', col2) → concatenate strings (MySQL)
...
Data cleaning is one of the most common analyst tasks — string functions make it systematic.`,
    ...
    realBusinessScenario: `A Swiggy analyst needs to group orders by month but the order_date column
      stores full dates like '2024-03-15'. There is no MONTH() function in MySQL, but SUBSTR(order_date, 1,
...
ORDER BY order_month;`,
        ...
        explanation:
          "SUBSTR(order_date, 1, 7) extracts 'YYYY-MM' from 'YYYY-MM-DD'. This is the " +
          "MySQL approach to monthly grouping.",
      },
...
LIMIT 10;`,
        ...
        explanation:
          "TRIM removes stray spaces. UPPER standardises city names. LENGTH helps spot " +
          "anomalies in text fields.",
      },
    ],

    commonMistakes: [
      "Using MONTH(date) in MySQL — it doesn't exist. Use SUBSTR(date, 6, 2) instead.",
    ...
    cheatSheet: [
      ...
      "CONCAT(col1, ' ', col2) → concatenate (MySQL)",
    ],
  },
...
In MySQL (which this course uses), dates are stored as TEXT in 'YYYY-MM-DD' format.

Key MySQL date operations:
...
Date arithmetic (MySQL):
  ...
  DATE_SUB(col, INTERVAL 1 MONTH)               → 1 month earlier

  DATEDIFF(date2) - DATEDIFF(date1) → number of days between
...
SUBSTR(signup_date, 1, 7) → '2023-06', '2023-07', etc.`,
    ...
    realBusinessScenario: `A Flipkart analyst needs monthly GMV trend for the last 6 months. They group
      ...
      signup) using DATEDIFF(CURDATE(), signup_date) to segment customers by age.`,
...
ORDER BY month;`,
        ...
        explanation:
          "Monthly GMV trend — a must-know query for any DA interview. GROUP BY the same " +
...
LIMIT 10;`,
        ...
        explanation:
          "DATEDIFF converts a date to a numeric value. The difference gives days between " +
          "two dates. Useful for cohort and retention analysis.",
      },
    ],

    commonMistakes: [
      "Using MySQL functions like DATE_FORMAT or YEAR() in MySQL — they don't exist.",
    ...
    interviewQuestions: [
      ...
      "What date functions would you use in BigQuery vs MySQL?",
    ],

    revisionNotes: [
      "In MySQL: use SUBSTR for date parts, DATEDIFF for differences.",
    ...
    cheatSheet: [
      "SUBSTR(date_col, 1, 7) → 'YYYY-MM' — month grouping (MySQL)",

      "DATEDIFF(d2, d1) → days between dates",
...
ORDER BY total_orders DESC;`,
        ...
        explanation:
          "GROUP BY status creates one row per status. COUNT(*) counts all orders in each " +
...
ORDER BY order_count DESC;`,
        ...
        explanation:
          "HAVING filters AFTER grouping. This finds repeat buyers — customers with more " +
...
ORDER BY net_revenue DESC;`,
        ...
        explanation:
          "This is a standard business report: revenue by region, split into gross and " +
...
FROM orders;`,
        ...
        explanation:
          "SUM(CASE WHEN ...) lets you compute multiple conditional aggregates in one " +
...
ORDER BY aov_net DESC;`,
        ...
        explanation:
          "We compute both AVG(gross) and manual AOV (net revenue / count). The difference " +
...
LIMIT 10;`,
        ...
        explanation:
          "NULLIF(total_amount, 0) prevents division by zero. CAST to REAL prevents integer division.",
      },
    ],
...
LIMIT 10;`,
        ...
        explanation:
          "This is a complete customer-level summary — a very common interview question. " +
...
ORDER BY price_range DESC;`,
        ...
        explanation:
          "Price range (MAX - MIN) tells you which category has the widest product spread " +
...
Advanced Aggregation (MySQL 8.0+):
...
ORDER BY c.region, month;`,
        ...
        explanation:
          "GROUP BY two columns: region and month. This creates one row per region-month " +
...
ORDER BY total_revenue DESC;`,
        ...
        explanation:
          "Which customer segment drives the most revenue? A classic segmentation question.",
      },
...
ORDER BY c.region, c.segment;`,
        ...
        explanation:
          "ROLLUP computes the revenue for each region+segment, then a sub-total for each " +
...
ORDER BY delivered_orders DESC;`,
        ...
        explanation:
          "WHERE filters to delivered orders (row level). HAVING then filters to cities " +
...
ORDER BY lifetime_value DESC;`,
        ...
        explanation:
          "Repeat HAVING pattern: filter groups on the aggregate. This identifies VIP " +
...
LIMIT 10;`,
        ...
        explanation:
          "INNER JOIN means only customers who have at least one delivered order appear. " +
...
LIMIT 10;`,
        ...
        explanation:
          "Three-table join: products → order_items → orders. Start from the most granular " +
...
ORDER BY c.signup_date;`,
        ...
        explanation:
          "LEFT JOIN keeps all customers. WHERE o.order_id IS NULL filters to only those " +
...
ORDER BY lifetime_value DESC;`,
        ...
        explanation:
          "Move the status filter into the JOIN condition (not WHERE) so that all " +
...
ORDER BY times_ordered ASC;`,
        ...
        explanation:
          "RIGHT JOIN ensures all products appear even if they have no order_items. " +
...
ORDER BY times_ordered ASC;`,
        ...
        explanation:
          "Identical result, but using LEFT JOIN with products as the leading table. This " +
...
Modern MySQL (3.39.0+) supports native RIGHT JOIN and FULL JOIN directly, similar to PostgreSQL and
...
ORDER BY o.order_id;`,
        ...
        explanation:
          "The UNION ALL combines a LEFT JOIN (all orders + matching payments) with a LEFT " +
...
ORDER BY order_ref;`,
        ...
        explanation:
          "COALESCE picks the first non-NULL value. In a full join, either side might be " +
    ...
    interviewQuestions: [
      ...
      "How do you simulate a FULL JOIN on older engines (like older MySQL or MySQL)?",
...
ORDER BY e1.department_id, e1.salary_lpa DESC;`,
        ...
        explanation:
          "LEFT JOIN ensures top-level managers (NULL manager_id) appear with NULL in the " +
...
ORDER BY e1.department_id;`,
        ...
        explanation:
          "e1.employee_id < e2.employee_id prevents both (A,B) and (B,A) from appearing, " +
...
ORDER BY delivered_orders DESC;`,
        ...
        explanation:
          "The inner subquery counts delivered orders for each specific customer_id from " +
...
ORDER BY oc.delivered_orders DESC;`,
        ...
        explanation:
          "This produces the same result but runs the average calculation only once " +
...
ORDER BY ltv.lifetime_value DESC;`,
        ...
        explanation:
          "CTE 1 cleans and filters. CTE 2 aggregates. Final SELECT joins to get customer " +
...
ORDER BY month;`,
        ...
        explanation:
          "CTE computes monthly revenue. Final SELECT uses LAG window function for " +
...
SELECT * FROM date_series;`,
        ...
        explanation:
          "This recursive CTE starts with Jan 1st. It then continually adds 1 day to the " +
...
ORDER BY revenue DESC;`,
        ...
        explanation:
          "Two-CTE pattern: (1) aggregate revenue per product, (2) rank with ROW_NUMBER " +
...
ORDER BY customer_id;`,
        ...
        explanation:
          "PARTITION BY customer_id restarts the row numbering for each customer. ORDER BY " +
...
ORDER BY category, rank_in_category;`,
        ...
        explanation:
          "PARTITION BY category creates separate windows per category. RANK() ranks " +
...
ORDER BY order_date;`,
        ...
        explanation:
          "SUM() OVER (ORDER BY date) = running total. AVG() OVER (ROWS BETWEEN 6 " +
...
ORDER BY month;`,
        ...
        explanation:
          "CTE aggregates to monthly grain. LAG(revenue,1) pulls the previous month's " +
...
ORDER BY customer_id, order_date;`,
        ...
        explanation:
          "PARTITION BY customer_id restarts LAG for each customer. ORDER BY order_date " +
          "means LAG looks back to the previous order for that same customer. DATEDIFF " +
...
FROM orders;`,
        ...
        explanation:
          "UNION ALL stacks the results of four independent queries. The last SELECT adds " +
...
ORDER BY signup_date DESC;`,
        ...
        explanation:
          "Stacks two filtered customer sets. UNION ALL is used (not UNION) because the " +
...
LIMIT 15;`,
        ...
        explanation:
          "CASE WHEN creates a categorical label from a numeric value. The tiers are " +
...
ORDER BY delivery_rate_pct DESC;`,
        ...
        explanation:
          "SUM(CASE WHEN ...) is the pivot technique. Each status becomes a column. This " +
...
ORDER BY revenue DESC;`,
        ...
        explanation:
          "The bad version puts a non-aggregate filter in HAVING (runs after full table " +
...
ORDER BY orders DESC;`,
        ...
        explanation:
          "EXPLAIN shows the database's plan without running the query. Look " +
    ...
    interviewQuestions: [
      ...
      "What does EXPLAIN or EXPLAIN tell you? What is the difference " +
    ...
    revisionNotes: [
      ...
      "EXPLAIN: shows index usage and scan type (interview win).",
...
ORDER BY city, rn;`,
        ...
        explanation:
          "CTE 1: aggregates LTV per customer. CTE 2: adds city average (AVG OVER " +
...
ORDER BY cf.cohort_month, om.order_month;`,
        ...
        explanation:
          "CTE 1: tags each customer with their signup cohort month. CTE 2: aggregates " +
...
FROM funnel;`,
        ...
        explanation:
          "Uses a LEFT JOIN to keep the top of the funnel (all signups), then uses COUNT " +
...
ORDER BY city;`,
        explanation:
          "IN checks if city matches any of the three values. Much cleaner than three OR conditions.",
      },
      {
...
ORDER BY total_amount;`,
        explanation:
          "BETWEEN 500 AND 2000 includes both endpoints. Combined with AND status filter.",
      },
      {
...
ORDER BY order_date DESC;`,
        explanation:
          "NOT IN excludes rows matching any listed value. Equivalent to status != " +
...
ORDER BY net_revenue DESC;`,
        explanation:
          "WHERE filters raw rows first (only Delivered orders). HAVING then filters the " +
...
ORDER BY avg_order_value DESC;`,
        explanation:
          "WHERE restricts to recent orders before grouping. HAVING keeps only customers " +
...
ORDER BY name;`,
        explanation:
          "Inner query finds customer IDs with qualifying orders. Outer query retrieves customer details.",
      },
      {
...
WHERE NOT EXISTS (
  ...
  WHERE o.customer_id = c.customer_id
        ...
        explanation:
          "Finds customers with no orders. NOT EXISTS is preferred over NOT IN because it " +
...
ORDER BY avg_orders_per_customer DESC;`,
        explanation:
          "Inner query computes orders per customer. Outer query averages those counts by " +
...
ORDER BY avg_orders_per_customer DESC;`,
        explanation:
          "Inner query gets orders-per-customer. Outer query averages those by city. This " +
...
ORDER BY revenue DESC;`,
        explanation:
          "Inner query ranks products by revenue within each category. Outer query filters " +
...
ORDER BY 1;`,
        explanation:
          "The outer SUM wraps the GROUP BY SUM to create a running total across months. " +
...
ORDER BY category_revenue DESC;`,
        explanation:
          "SUM(oi.quantity * oi.unit_price) gives each category's revenue. SUM(...) OVER " +
...
ORDER BY 1;`,
        explanation:
          "ROWS BETWEEN 2 PRECEDING AND CURRENT ROW creates a 3-month window (2 before + " +
...
WHERE order_id IN (SELECT order_id FROM orders WHERE order_date LIKE '2024-02%');`,
        explanation:
          "Returns product IDs that appear in BOTH January AND February orders.",
      },
      {
...
SELECT DISTINCT customer_id FROM orders WHERE status = 'Returned';`,
        explanation:
          "Customers who placed a Delivered order but have NO Returned order. EXCEPT " +
...
WHERE o1.status = 'Delivered'
  AND o2.customer_id IS NULL;`,
        explanation:
          "This produces the same result as the EXCEPT query above. LEFT JOIN + IS NULL is " +
...
ORDER BY month;`,
        explanation:
          "CASE WHEN inside SUM() routes each row's revenue to the right column. ELSE 0 prevents NULL.",
      },
      {
...
ORDER BY lifetime_value DESC;`,
        explanation:
          "NTILE(4) divides customers by revenue into 4 equal buckets. Bucket 1 = top 25% (highest LTV).",
      },
    ],
...
- reward_amount (must be >= 0)`,
  ...
  created_at  DATE DEFAULT (CURDATE())
        ...
        explanation:
          "PRIMARY KEY ensures unique customer IDs. UNIQUE NOT NULL on email prevents " +
  ...
  category     VARCHAR(50) NOT NULL
        ...
        explanation:
          "CHECK constraints enforce business rules at the database level. price > 0 " +
...
3. Rename a column (MySQL 3.25+):
   ALTER TABLE table_name RENAME COLUMN old_name TO new_name;

4. Drop a column (not supported in older MySQL):
   ALTER TABLE table_name DROP COLUMN column_name;

MySQL limitation: MySQL supports ADD COLUMN and RENAME, but NOT change of data type
...
ALTER TABLE customer_orders RENAME TO orders;`,
      ...
      {
        title: "Recreate table for complex changes (MySQL pattern)",
...
ALTER TABLE customers_new RENAME TO customers;`,
        explanation:
          "When MySQL doesn't support the change directly, use this create-copy-drop-rename pattern.",
      },
    ],
    ...
    interviewQuestions: [
      ...
      "What is the limitation of ALTER TABLE in MySQL?",
    ...
    revisionNotes: [
      ...
      "MySQL complex changes: create new + INSERT SELECT + DROP old + RENAME",
    ],
...
VALUES
  ...
  (202, 'Yoga Mat', 799.00, 'Sports');`,
        explanation:
          "INSERT VALUES adds specific rows. Multi-row INSERT is more efficient than individual INSERTs.",
      },
      {
...
WHERE status = 'Pending'
  AND order_date < '2024-01-01';`,
        explanation:
          "Always SELECT first to verify the WHERE condition before running UPDATE. The " +
...
WHERE status = 'Cancelled'
  AND order_date < '2023-01-01';`,
        explanation:
          "Check row count first. Delete is irreversible without a transaction — use " +
...
Limitations:
  ...
  - For performance, use Materialized Views (available in PostgreSQL, Snowflake, BigQuery — NOT in MySQL)
...
ORDER BY revenue DESC;`,
        explanation:
          "The view hides the JOIN complexity. Users query delivered_orders like a simple table.",
      },
      {
...
CREATE UNIQUE INDEX idx_customers_email ON customers (email);`,
      ...
      {
        title: "EXPLAIN to verify index use",
...
-- If it shows SCAN TABLE orders -- index not being used, check your query`,
        explanation:
          "EXPLAIN shows if the database uses your index. SEARCH = fast index " +
...
SCAN TABLE vs SEARCH TABLE (MySQL):
...
4. Re-run EXPLAIN to verify SCAN → SEARCH`,
  ...
  → Very fast, always fine.`,
    ...
    realBusinessScenario: `A data analyst at Paytm reports that a dashboard query takes 20 seconds.
      Running EXPLAIN reveals: "SCAN TABLE transactions" — the query filters on transaction_date
...
WHERE o.status = 'Delivered'
  AND o.order_date >= '2024-01-01';`,
        explanation:
          "Without an index on status/order_date, this will show SCAN TABLE orders. Adding " +
...
WHERE o.status = 'Delivered'
  AND o.order_date >= '2024-01-01';`,
        explanation:
          "After the index is created, the query planner uses it and shows SEARCH instead of SCAN.",
      },
    ],
    ...
    revisionNotes: [
      ...
      "Use EXPLAIN before optimising a slow query",
    ],

    cheatSheet: [
      "EXPLAIN SELECT ... — see the execution plan (MySQL)",
...
-- If something's wrong: ROLLBACK;`,
        explanation:
          "Run SELECT after UPDATE to verify the result before COMMIT. ROLLBACK is your safety net.",
      },
    ],
  ...
  42: {
    conceptExplanation:
      "CTAS stands for 'CREATE TABLE AS SELECT'. It is a powerful, concise DDL " +
    ...
    visualExplanation:
      "Think of a standard CREATE TABLE + INSERT INTO as building a bookshelf " +
      ...
      "into a new shelf.",
    realBusinessScenario:
      "You are a Growth Analyst at a ride-sharing startup. The database contains " +
    ...
    examples: [
      {
        ...
        explanation:
          "This creates a new table named 'high_value_customers' with columns " +
    ...
    commonMistakes: [
      ...
      "Not aliasing calculated columns (e.g. SUM(amount)) in the SELECT statement — " +
        "MySQL requires aliases for expressions in CTAS.",
    ],
    interviewQuestions: [
    ...
    cheatSheet: [
      ...
      "MySQL requires all SELECT columns to have names (use AS alias).",
    ],
  },
  43: {
    conceptExplanation:
      "Temporary Tables are tables that exist only for the duration of the current " +
      ...
      "private workspaces without cluttering the global database schema. In MySQL, " +
      "they are created using the CREATE TEMPORARY TABLE (or CREATE TEMP TABLE) syntax.",
    visualExplanation:
      "A permanent table is like writing a document in a leather-bound notebook. A " +
    ...
    realBusinessScenario:
      "You are performing a complex revenue attribution analysis. To calculate the " +
    ...
    examples: [
      {
        ...
        explanation:
          "Creates a session-specific temporary table storing active subscriptions and " +
    ...
    revisionNotes: [
      ...
      "They are stored in a separate temporary database schema (e.g. 'temp' in MySQL).",
    ],
    cheatSheet: [
  ...
  return {
    ...
    examples: [
...

      {
        ...
        query: defaultQuery.replace(
          "ORDER BY",
          "-- Interviewers expect you to explain the logic\nORDER BY",
        ),
...
export function getLevel(id: number): ModuleLevel {
...
export const moduleOutcomes: Record<number, string> = {
...
export const modulePrerequisites: Record<number, number[]> = {
  2: [1],
  3: [1, 2],
  4: [1, 2],
  5: [1, 2],
...
export const validateUniqueProblemIds = (modules: RoadmapModule[]) => {
  ...
  if (duplicates.length > 0) {
    throw new Error(
      `[Curriculum Integrity Error] Duplicate problem IDs detected: ${duplicates.join(", ")}`,
    );
  }
};

...
export const debugPuzzles: SqlPuzzle[] = [
...

  {
    ...
    businessScenario:
      "Find all customers who have placed more than 2 orders. The junior analyst wrote this query, but it crashes.",
...
GROUP BY customer_id;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The WHERE vs HAVING Trap'.",
      "Group your output rows using GROUP BY and apply aggregate functions like COUNT(), SUM(), or AVG().",
      "Structure your query using clauses similar to: SELECT customer_id, COUNT(order_id) as total_orders.",
    ],
...
HAVING COUNT(order_id) > 2;`,
    ...
    mistakeExplanation:
      "You cannot use aggregate functions like COUNT() in a WHERE clause. WHERE filters individual rows before they are grouped. To filter groups based on an aggregate, you must use the HAVING clause AFTER the GROUP BY.",
  },
  {
    id: "pz-2",
    dayId: 8,
...
FROM orders;`,
  ...
  },
  {
    ...
    category: "Aggregation",
    businessScenario:
      "We want to see the total revenue for each customer segment. But the database is throwing an error about unaggregated columns.",
...
GROUP BY segment;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The GROUP BY Ghost'.",
      "Connect the primary table with related foreign key tables using INNER JOIN or LEFT JOIN.",
      "Structure your query using clauses similar to: SELECT segment, city, SUM(total_amount).",
    ],
...
GROUP BY segment, city;`,
    ...
    mistakeExplanation:
      "Every non-aggregated column in your SELECT clause MUST be included in your GROUP BY clause. Since you selected 'city' but didn't group by it, the database doesn't know which city to display for the segment.",
  },
  {
    id: "pz-4",
    dayId: 8,
...
GROUP BY d.department_name;`,
  ...
  },
  {
    ...
    category: "Aggregation",
    businessScenario:
      "We want to find the overall average rating of food orders. The current math calculates it manually but ignores null ratings incorrectly.",
    flawedQuery: `-- Manual average returns the wrong value when rating is null
SELECT SUM(rating) / COUNT(*) as manual_average
FROM food_orders;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'Average Calculation Loophole'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT AVG(rating) as manual_average.",
    ],
...
FROM food_orders;`,
    ...
    mistakeExplanation:
      "COUNT(*) counts all rows, including those where rating is NULL, resulting in a denominator that is too large. AVG(rating) automatically ignores NULL values in both the sum and the row count, matching the correct mathematical average.",
  },
  {
    id: "pz-8",
    dayId: 9,
    ...
    category: "Aggregation",
    businessScenario:
      "Find the maximum product list price in each product category. Right now, it outputs a row for every single product instead of summarizing by category.",
...
GROUP BY category;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Over-Grouping Leak'.",
      "Group your output rows using GROUP BY and apply aggregate functions like COUNT(), SUM(), or AVG().",
      "Structure your query using clauses similar to: SELECT category, MAX(list_price) as max_price.",
    ],
...
GROUP BY category;`,
    ...
    mistakeExplanation:
      "Including product_name in the SELECT clause without an aggregate function or grouping by it violates grouping rules (or returns arbitrary values in some SQL versions). Grouping should only select the grouped keys and their aggregates.",
  },
  {
    id: "pz-9",
    dayId: 10,
    ...
    category: "Aggregation",
    businessScenario:
      "We want to count how many orders were successfully delivered versus returned. The query crashes.",
...
FROM orders;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Conditional Sum Muddle'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
...
FROM orders;`,
    ...
    mistakeExplanation:
      "Every CASE expression must terminate with the END keyword. Without it, the SQL compiler cannot parse where the condition evaluation finishes.",
  },
  {
    id: "pz-10",
    dayId: 10,
...
FROM orders;`,
    ...
    mistakeExplanation:
      "SUM(discount_amount / total_amount) calculates the ratio for each row and then sums them up, which is mathematically incorrect for an overall ratio. You must divide the sum of all discounts by the sum of all revenues: `SUM(discount) / SUM(total)`.",
  },
...

  {
    ...
    businessScenario:
      "We want a list of ALL customers, along with their order IDs (if they have any). If they haven't ordered, order_id should be NULL. The current query only returns customers who HAVE ordered.",
...
WHERE o.status = 'Delivered';`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Phantom Inner Join'.",
      "Connect the primary table with related foreign key tables using INNER JOIN or LEFT JOIN.",
      "Structure your query using clauses similar to: SELECT c.full_name, o.order_id.",
    ],
...
LEFT JOIN orders o ON c.customer_id = o.customer_id AND o.status = 'Delivered';`,
    ...
    mistakeExplanation:
      "Putting a condition on the right table (orders) in the WHERE clause filters out all rows where that table's columns are NULL (because NULL = 'Delivered' is not true). This turns the LEFT JOIN into an INNER JOIN. Move the condition into the ON clause.",
  },
  {
    id: "pz-12",
    dayId: 13,
    ...
    category: "JOINs & Subqueries",
    businessScenario:
      "We want to see each product category and its total revenue. But the numbers are in the trillions and the query took forever!",
    flawedQuery: `-- Why are the revenue numbers insanely high?
SELECT p.category, SUM(o.total_amount) as rev
FROM products p, orders o
GROUP BY p.category;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Cross Join Catastrophe'.",
      "Connect the primary table with related foreign key tables using INNER JOIN or LEFT JOIN.",
      "Structure your query using clauses similar to: SELECT p.category, SUM(oi.quantity * oi.unit_price) as rev.",
    ],
...
GROUP BY p.category;`,
    ...
    mistakeExplanation:
      "Selecting from two tables separated by a comma without a JOIN condition creates a CROSS JOIN (Cartesian product). Every product is joined with every order, causing exponential row growth. Always use explicit JOIN ... ON syntax.",
  },
  {
    id: "pz-13",
    dayId: 13,
...
FROM (
  ...
  GROUP BY customer_id
    ...
    mistakeExplanation:
      "In SQL, a subquery used in a FROM clause represents a temporary table. Standard SQL requires every derived table to have an alias name (e.g. `) sub;`), even if it is not explicitly referenced.",
  },
  {
    id: "pz-14",
    dayId: 14,
...
WHERE customer_id NOT IN (
  SELECT DISTINCT customer_id FROM orders WHERE customer_id IS NOT NULL
    ...
    mistakeExplanation:
      "In SQL, if a subquery in a NOT IN clause returns a NULL, the overall expression evaluates to UNKNOWN, causing the parent query to return empty results. You must filter out NULLs in the subquery or use NOT EXISTS.",
  },
  {
    id: "pz-15",
    dayId: 14,
    ...
    category: "JOINs & Subqueries",
    businessScenario:
      "We want to see the count of orders for each customer. The query below shows inflated, incorrect counts.",
...
GROUP BY c.full_name;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Duplicating JOIN Multiplier'.",
      "Connect the primary table with related foreign key tables using INNER JOIN or LEFT JOIN.",
      "Structure your query using clauses similar to: SELECT c.full_name, COUNT(DISTINCT o.order_id) as order_count.",
    ],
...
GROUP BY c.full_name;`,
    ...
    mistakeExplanation:
      "Joining `order_items` multiplies the rows for each order (one row per item). Thus, COUNT(o.order_id) counts each order once per item. Remove the redundant join or use COUNT(DISTINCT o.order_id).",
  },
  {
    id: "pz-16",
    dayId: 14,
    ...
    category: "JOINs & Subqueries",
    businessScenario:
      "Find customers who ordered, but the compiler complains about an ambiguous column ID.",
    flawedQuery: `-- Column customer_id is ambiguous!
SELECT customer_id, full_name, order_date
FROM customers c
JOIN orders o ON c.customer_id = o.customer_id;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Ambiguous Column Ambush'.",
      "Connect the primary table with related foreign key tables using INNER JOIN or LEFT JOIN.",
      "Structure your query using clauses similar to: SELECT c.customer_id, c.full_name, o.order_date.",
    ],
...
JOIN orders o ON c.customer_id = o.customer_id;`,
    ...
    mistakeExplanation:
      "When two tables in a JOIN contain columns with identical names, you must prefix the column with the table name or alias (e.g. `c.customer_id`) so the SQL engine knows which column to return.",
  },
  {
    id: "pz-17",
    dayId: 14,
    ...
    category: "JOINs & Subqueries",
    businessScenario:
      "Find all customers who have never placed an order using a LEFT JOIN. The query runs but returns no rows.",
...
WHERE o.order_id = NULL;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Left Join Null Comparison'.",
      "Connect the primary table with related foreign key tables using INNER JOIN or LEFT JOIN.",
      "Structure your query using clauses similar to: SELECT c.customer_id, c.full_name.",
    ],
...
WHERE o.order_id IS NULL;`,
    ...
    mistakeExplanation:
      "In SQL, comparisons with NULL using '=' or '!=' always return false/unknown. You must use the `IS NULL` or `IS NOT NULL` operators to check for null values.",
  },
  {
    id: "pz-18",
    dayId: 15,
    ...
    category: "JOINs & Subqueries",
    businessScenario:
      "We want to see employee names along with their manager names. But right now it is returning managers' IDs instead of manager names.",
    flawedQuery: `-- This shows managers' employee IDs, not manager names!
SELECT e.employee_name, e.manager_id as manager_name
FROM employees e;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The HR Reporting Loop'.",
      "Connect the primary table with related foreign key tables using INNER JOIN or LEFT JOIN.",
      "Structure your query using clauses similar to: SELECT e.employee_name, m.employee_name as manager_name.",
    ],
...
LEFT JOIN employees m ON e.manager_id = m.employee_id;`,
    ...
    mistakeExplanation:
      "To find employee relationships in a single hierarchy table, you must perform a self-join. Join the table to itself using different aliases (e.g. `employees e` and `employees m`) on `e.manager_id = m.employee_id`.",
  },
  {
    id: "pz-19",
    dayId: 18,
    ...
    category: "JOINs & Subqueries",
    businessScenario:
      "We want to check if a customer has placed an order with gross value > 10000. The subquery tries to select all customer records, making it extremely slow and incorrect.",
...
WHERE EXISTS (
  SELECT * FROM orders o WHERE o.total_amount > 10000
    ...
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Inefficient Subquery Check'.",
      "Filter your result set using WHERE conditions to isolate the target criteria.",
      "Structure your query using clauses similar to: SELECT c.full_name.",
    ],
...
WHERE EXISTS (
  SELECT 1 FROM orders o WHERE o.customer_id = c.customer_id AND o.total_amount > 10000
    ...
    mistakeExplanation:
      "A standard EXISTS subquery must be 'correlated' to the outer query. Without linking `o.customer_id = c.customer_id`, the EXISTS block returns true for everyone if *any* customer has placed an order > 10000.",
  },
  {
    id: "pz-20",
    dayId: 18,
...
SELECT office_city FROM departments;`,
...

  {
    ...
    businessScenario:
      "We want to rank the highest-value orders PER REGION. But right now, it's just ranking all orders globally from 1 to 50.",
...
JOIN customers c ON o.customer_id = c.customer_id;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Window Function Wipeout'.",
      "Connect the primary table with related foreign key tables using INNER JOIN or LEFT JOIN.",
      "Structure your query using clauses similar to: SELECT c.region, o.order_id, o.total_amount,.",
    ],
...
JOIN customers c ON o.customer_id = c.customer_id;`,
    ...
    mistakeExplanation:
      "Without a PARTITION BY clause, the window function treats the entire result set as a single group. To calculate metrics 'per group' (like per region), you must use PARTITION BY inside the OVER() clause.",
  },
  {
    id: "pz-22",
    dayId: 21,
    ...
    category: "Window Functions",
    businessScenario:
      "We want to calculate a running total of payments over time. However, payments on the same date are being added together in a lump sum instead of step-by-step.",
...
FROM payments;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Running Total Tie Trap'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT payment_id, payment_date, amount,.",
    ],
...
FROM payments;`,
    ...
    mistakeExplanation:
      "When duplicate values exist in the window ORDER BY clause, the engine groups them and sums them in a single step. To get a true cumulative sum, append a unique column (e.g. payment_id) to make the sorting key unique.",
  },
  {
    id: "pz-23",
    dayId: 21,
    ...
    category: "Window Functions",
    businessScenario:
      "We want to find only the highest-paying employee per department. The query crashes.",
...
WHERE RANK() OVER (PARTITION BY department_id ORDER BY salary_lpa DESC) = 1;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Filtering Windows Trap'.",
      "Filter your result set using WHERE conditions to isolate the target criteria.",
      "Structure your query using clauses similar to: SELECT employee_name, department_id, salary_lpa.",
    ],
...
WHERE rnk = 1;`,
    ...
    mistakeExplanation:
      "Because of SQL execution order, window functions are evaluated in the SELECT phase, which happens after the WHERE clause. Thus, you cannot filter on rank inside the same query's WHERE clause. Wrap it in a subquery/CTE.",
  },
  {
    id: "pz-24",
    dayId: 22,
    ...
    category: "Window Functions",
    businessScenario:
      "We want a running average of payment amounts over time. We want it row-by-row, but duplicates cause incorrect averages.",
...
FROM payments;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Range Default Loophole'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT payment_date, amount,.",
    ],
...
FROM payments;`,
    ...
    mistakeExplanation:
      "By default, an ORDER BY clause in a window function uses the RANGE frame, which aggregates all duplicate values of the sorting key together. Specifying ROWS BETWEEN makes the aggregation row-by-row.",
  },
  {
    id: "pz-25",
    dayId: 22,
...
FROM orders;`,
    hints: [
      ...
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT order_id, total_amount,.",
    ],
...
FROM orders;`,
    ...
    mistakeExplanation:
      "LAG and LEAD require an ordered sequence to identify preceding/following rows. Without an ORDER BY clause inside the OVER() container, the row ordering is arbitrary and values will be incorrect.",
  },
  {
    id: "pz-26",
    dayId: 22,
...
FROM customers;`,
    hints: [
      ...
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT customer_id, segment, signup_date,.",
    ],
...
FROM customers;`,
    ...
    mistakeExplanation:
      "By default, the window frame is `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`. This prevents FIRST_VALUE from seeing past the current row when evaluating later items. Expand it to `UNBOUNDED FOLLOWING`.",
  },
  {
    id: "pz-28",
    dayId: 24,
    ...
    category: "Window Functions",
    businessScenario:
      "Rank products by cost price within their category. The query crashes due to syntax placement error.",
...
FROM products;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Partition Sort Mismatch'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT category, product_name, cost_price,.",
    ],
...
FROM products;`,
    ...
    mistakeExplanation:
      "The SQL syntax requires PARTITION BY to precede the ORDER BY clause inside the OVER(...) container. Inverting their order results in a compiler syntax error.",
  },
  {
    id: "pz-29",
    dayId: 24,
    ...
    category: "Window Functions",
    businessScenario:
      "Distribute employee salaries into 4 equal quartiles. But the ranks are scrambled because of sorting omission.",
    flawedQuery: `-- Employees are randomly assigned to buckets
SELECT employee_name, salary_lpa,
  NTILE(4) OVER (PARTITION BY department_id) as salary_quartile
FROM employees;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The NTile Division Gap'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT employee_name, salary_lpa,.",
    ],
    solutionQuery: `SELECT employee_name, salary_lpa,
...
FROM employees;`,
    ...
    mistakeExplanation:
      "NTILE(N) requires an ORDER BY clause inside the window definition to group elements progressively. Without sorting, rows are assigned to buckets in arbitrary order.",
  },
  {
    id: "pz-30",
    dayId: 24,
    ...
    category: "Window Functions",
    businessScenario:
      "We want to see the date difference between the start of a subscription and the previous subscription's end date. The query crashes.",
...
FROM subscriptions;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Churn Date Difference'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT subscription_id, start_date,.",
    ],
    solutionQuery: `SELECT subscription_id, start_date,
  DATEDIFF(start_date, LAG(end_date) OVER (ORDER BY start_date)) AS days_between
FROM subscriptions;`,
    hint: "In MySQL, use DATEDIFF() to subtract dates, and remember to sort inside the OVER clause.",
    mistakeExplanation:
      "MySQL's DATEDIFF() returns the number of days between two dates. Furthermore, all offset functions like LAG() require sorting inside the OVER clause to establish preceding rows.",
  },
...
WHERE (total_amount - discount_amount) > 1000;`,
  ...
  },
  {
    ...
    category: "Syntax & Logic",
    businessScenario:
      "Find customers who signed up in 2023. The query executes but returns 0 rows.",
    flawedQuery: `-- Why are 2023 signups missing?
SELECT customer_id, full_name, signup_date
FROM customers
WHERE signup_date = '18-01-2023';`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Date Format Hazard'.",
      "Filter your result set using WHERE conditions to isolate the target criteria.",
      "Structure your query using clauses similar to: SELECT customer_id, full_name, signup_date.",
    ],
...
WHERE signup_date = '2023-01-18';`,
    ...
    mistakeExplanation:
      "MySQL and most relational databases require date literals in the ISO YYYY-MM-DD format. Querying with DD-MM-YYYY fails to match standard date fields.",
  },
  {
    id: "pz-33",
    dayId: 11,
...
FROM products;`,
    hints: [
      ...
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT product_name,.",
    ],
...
FROM products;`,
    ...
    mistakeExplanation:
      "Dividing by zero causes database engine crashes. The SQL function NULLIF(col, 0) returns NULL if the column is 0, and dividing by NULL safely yields NULL instead of crashing.",
  },
  {
    id: "pz-34",
    dayId: 25,
    ...
    category: "Syntax & Logic",
    businessScenario:
      "We want to find all customers whose segment name starts with 'Pr'. The query runs but returns no rows.",
    flawedQuery: `-- Why is this returning 0 rows?
SELECT customer_id, full_name, segment
FROM customers
WHERE segment LIKE 'Pr_';`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Bad Wildcard Matcher'.",
      "Filter your result set using WHERE conditions to isolate the target criteria.",
      "Structure your query using clauses similar to: SELECT customer_id, full_name, segment.",
    ],
...
WHERE segment LIKE 'Pr%';`,
    ...
    mistakeExplanation:
      "In SQL LIKE clauses, '_' matches exactly one character (e.g. 'Pr_' matches 'Pro' but not 'Premium'). The '%' wildcard matches zero or more characters.",
  },
  {
    id: "pz-35",
    dayId: 25,
...
FROM employees;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Case Statement Override'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT employee_name, salary_lpa,.",
    ],
    solutionQuery: `SELECT employee_name, salary_lpa,
...
FROM employees;`,
  ...
  },
  {
    ...
    category: "Syntax & Logic",
    businessScenario:
      "We want to calculate the actual amount paid (total_amount minus discount_amount). But if there is no discount (discount_amount is NULL), the calculation returns NULL instead of the total_amount.",
    flawedQuery: `-- Orders with no discount are returning NULL paid_amount!
SELECT order_id,
  (total_amount - discount_amount) as paid_amount
FROM orders;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Null Revenue Leak'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT order_id,.",
    ],
    solutionQuery: `SELECT order_id,
...
FROM orders;`,
    ...
    mistakeExplanation:
      "Performing subtraction with NULL (e.g. `1000 - NULL`) returns NULL. Wrap the nullable column in COALESCE(discount_amount, 0) to treat NULL as 0.",
  },
  {
    id: "pz-37",
    dayId: 27,
    ...
    category: "Syntax & Logic",
    businessScenario:
      "We want to merge name and city into a label (e.g., 'Aarav Mehta (Mumbai)'). The query returns numbers or zeros.",
    flawedQuery: `-- This returns 0 or numbers in MySQL!
SELECT (full_name + ' (' + city + ')') as display_label
FROM customers;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Plus Concatenation Trap'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT CONCAT(full_name,.",
    ],
    solutionQuery: `SELECT CONCAT(full_name, ' (', city, ')') AS display_label
FROM customers;`,
    hint: "In MySQL, use CONCAT() to concatenate strings. What happens when '+' is used instead?",
    mistakeExplanation:
      "MySQL uses CONCAT() for string concatenation. Using '+' performs numeric conversion, which can turn text into 0 and produce an incorrect label.",
  },
  {
    id: "pz-38",
    dayId: 28,
    ...
    category: "Syntax & Logic",
    businessScenario:
      "Find customers from 'Mumbai' or 'Delhi' who are in the 'Premium' segment. But the query is returning 'Student' and 'Value' customers from Delhi.",
...
WHERE city = 'Mumbai' OR city = 'Delhi' AND segment = 'Premium';`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Logic Operator Priority'.",
      "Filter your result set using WHERE conditions to isolate the target criteria.",
      "Structure your query using clauses similar to: SELECT full_name, city, segment.",
    ],
...
WHERE (city = 'Mumbai' OR city = 'Delhi') AND segment = 'Premium';`,
    ...
    mistakeExplanation:
      "The logical AND operator is evaluated before OR. So, the query reads as: '(Mumbai customers) OR (Delhi customers who are Premium)'. Group OR conditions in brackets.",
  },
  {
    id: "pz-39",
    dayId: 28,
    ...
    category: "Syntax & Logic",
    businessScenario:
      "We want to find active subscriptions. The query crashes or returns 0 rows.",
    flawedQuery: `-- Syntax error checking boolean values
SELECT subscription_id, status
FROM subscriptions
WHERE status = TRUE;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Boolean String Hazard'.",
      "Filter your result set using WHERE conditions to isolate the target criteria.",
      "Structure your query using clauses similar to: SELECT subscription_id, status.",
    ],
...
WHERE status = 'Active';`,
    ...
    mistakeExplanation:
      "The status column stores text values ('Active', 'Paused', 'Churned') rather than Boolean flags. Compare it to the string value `'Active'`.",
  },
  {
    id: "pz-40",
    dayId: 4,
    ...
    category: "Syntax & Logic",
    businessScenario:
      "We want to find all subscriptions that have ended (end_date is not null). The query returns no rows.",
    flawedQuery: `-- Why is this returning 0 rows?
SELECT subscription_id, end_date
FROM subscriptions
WHERE end_date != NULL;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Inverted Null Compare'.",
      "Filter your result set using WHERE conditions to isolate the target criteria.",
      "Structure your query using clauses similar to: SELECT subscription_id, end_date.",
    ],
...
WHERE end_date IS NOT NULL;`,
    ...
    mistakeExplanation:
      "Null represents the absence of a value, so it cannot be compared using standard comparison operators. Always check for non-null values using `IS NOT NULL`.",
  },
  {
    id: "pz-41",
    dayId: 8,
    ...
    category: "Aggregation",
    businessScenario:
      "We want to find the number of unique customers who placed successful orders vs. cancelled orders. But the conditional counts are returning incorrect values because of a common DISTINCT syntax oversight.",
...
FROM orders;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Conditional Count DISTINCT Trap'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT.",
    ],
    solutionQuery: `SELECT
...
FROM orders;`,
    ...
    mistakeExplanation:
      "COUNT(DISTINCT case_expression) is the correct syntax. Putting DISTINCT inside the CASE WHEN ... THEN block is invalid SQL syntax.",
  },
  {
    id: "pz-42",
    dayId: 8,
...
FROM orders;`,
  ...
  },
  {
    ...
    category: "JOINs & Subqueries",
    businessScenario:
      "We want to see each customer's details alongside their total lifetime order amount and total payments. But the current query returns duplicate-multiplied inflated values.",
...
GROUP BY c.customer_id, c.full_name;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Aggregated Join Inflater'.",
      "Connect the primary table with related foreign key tables using INNER JOIN or LEFT JOIN.",
      "Structure your query using clauses similar to: WITH order_summary AS (.",
    ],
...
LEFT JOIN payment_summary ps ON c.customer_id = ps.customer_id;`,
    ...
    mistakeExplanation:
      "When you join a table to multiple child tables (orders and payments), rows are multiplied, causing aggregates like SUM() to double-count. The solution is to calculate totals inside separate CTEs first, and then join those summarized results back to the customer table.",
  },
  {
    id: "pz-44",
    dayId: 18,
    ...
    category: "JOINs & Subqueries",
    businessScenario:
      "Find all products whose list price is greater than the average list price of products in their own category. The current query returns only products above the global average.",
...
ORDER BY category;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Correlated Subquery Trap'.",
      "Filter your result set using WHERE conditions to isolate the target criteria.",
      "Structure your query using clauses similar to: SELECT product_name, category, list_price.",
    ],
...
ORDER BY category;`,
    ...
    mistakeExplanation:
      "The flawed query compares each product to the overall database average. To make it a correlated subquery that compares each product to its own category average, you must add WHERE p2.category = p1.category inside the subquery.",
  },
  {
    id: "pz-45",
    dayId: 22,
    ...
    category: "Window Functions",
    businessScenario:
      "Find the latest signup date for each customer segment. The analyst used LAST_VALUE() sorted by signup_date, but it returns the current row's date.",
...
FROM customers;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The LAST_VALUE Frame Trap'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT customer_id, segment, signup_date,.",
    ],
    solutionQuery: `SELECT customer_id, segment, signup_date,
...
FROM customers;`,
    ...
    mistakeExplanation:
      "By default, using ORDER BY sets the window frame to RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW. This means LAST_VALUE can never see past the current row. To fix it, you must explicitly define the frame to search all the way to UNBOUNDED FOLLOWING.",
  },
  {
    id: "pz-46",
    dayId: 22,
    ...
    category: "Window Functions",
    businessScenario:
      "We want to see the total sales per city, along with the percentage contribution of each customer. The query crashes.",
...
GROUP BY customer_id;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Aggregated Window Crash'.",
      "Connect the primary table with related foreign key tables using INNER JOIN or LEFT JOIN.",
      "Structure your query using clauses similar to: SELECT c.city, c.customer_id,.",
    ],
...
GROUP BY c.city, c.customer_id;`,
    ...
    mistakeExplanation:
      "The query selects city but only groups by customer_id. Because city is not aggregated, this violates standard grouping rules. You must group by both c.city and c.customer_id for the window aggregation to execute correctly.",
  },
  {
    id: "pz-47",
    dayId: 30,
    ...
    category: "Syntax & Logic",
    businessScenario:
      "Calculate the refund rate for each payment mode (refunded amount divided by total amount * 100). The query executes, but it returns 0.0 for all payment modes instead of the decimal percentage!",
...
GROUP BY payment_mode;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Integer Division Zero Leak'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT payment_mode,.",
    ],
...
GROUP BY payment_mode;`,
    ...
    mistakeExplanation:
      "Because both columns are integers, the division operator performs integer division, truncating the fractional part to 0 before multiplying by 100. Multiplying the numerator by 100.0 first converts it to a floating-point number, ensuring decimal accuracy.",
  },
  {
    id: "pz-48",
    dayId: 4,
    ...
    category: "Syntax & Logic",
    businessScenario:
      "Generate a full address label: city followed by the region in parentheses. But for customers where region is missing (NULL), the entire label evaluates to NULL!",
    flawedQuery: `-- Null region wipes out the entire label
SELECT full_name,
  CONCAT(city, ' (', region, ')') AS location_label
FROM customers;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The NULL Concatenation Hazard'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT full_name,.",
    ],
    solutionQuery: `SELECT full_name,
  CONCAT(city, ' (', COALESCE(region, 'Unknown'), ')') AS location_label
FROM customers;`,
    hint: "In SQL, any string concatenation with a NULL value evaluates to NULL. How do you substitute a default value for NULL?",
    mistakeExplanation:
      "MySQL CONCAT() returns NULL when any argument is NULL. Wrap the nullable region column in COALESCE(region, 'Unknown') so it defaults to a fallback string instead of destroying the entire label.",
  },
  {
    id: "pz-49",
    dayId: 10,
    ...
    category: "Aggregation",
    businessScenario:
      "We want to see daily transaction summaries from our food delivery operations. However, grouping by order_date (which represents raw dates) should still be cleaned and aggregated daily.",
    flawedQuery: `-- Clean and aggregate food orders count by date
SELECT order_date, COUNT(*) as orders_count
FROM food_orders
GROUP BY order_date;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'Timestamp Truncation Grouping'.",
      "Group your output rows using GROUP BY and apply aggregate functions like COUNT(), SUM(), or AVG().",
      "Structure your query using clauses similar to: SELECT DATE(order_date) as order_date, COUNT(*) as orders_count.",
    ],
...
GROUP BY DATE(order_date);`,
    ...
    mistakeExplanation:
      "To aggregate daily metrics robustly, wrap the order_date in the DATE() function in both the SELECT list and the GROUP BY clause to normalize values.",
  },
  {
    id: "pz-50",
    dayId: 29,
    ...
    category: "Syntax & Logic",
    businessScenario:
      "A finance analyst wants to audit transactions that are exactly ₹99.99. But comparing float columns using '=' fails or behaves unpredictably due to floating-point precision.",
    flawedQuery: `-- Equality on floating point columns is unreliable
SELECT payment_id, amount
FROM payments
WHERE amount = 99.99;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The FLOAT Equality Precision Bug'.",
      "Filter your result set using WHERE conditions to isolate the target criteria.",
      "Structure your query using clauses similar to: SELECT payment_id, amount.",
    ],
...
WHERE ROUND(amount, 2) = 99.99;`,
    ...
    mistakeExplanation:
      "Floating-point numbers (REAL/FLOAT) represent fractional values using binary approximations, which can cause tiny precision mismatches. Directly comparing floats using '=' can result in missed records. Wrap the column in ROUND(col, 2) to perform a safe comparison.",
  },
  {
    id: "pz-51",
    dayId: 22,
    ...
    category: "Window Functions",
    businessScenario:
      "We want to see each employee's name and salary alongside the total salary budget for their department. But the department total column behaves like a running sum instead of a static total.",
...
FROM employees;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Accidental Running Sum'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT employee_name, salary_lpa,.",
    ],
    solutionQuery: `SELECT employee_name, salary_lpa,
...
FROM employees;`,
    ...
    mistakeExplanation:
      "In a window function, adding an ORDER BY clause inside the OVER() block alters the default frame to compute a cumulative (running) total. To get a static group total, omit the ORDER BY clause.",
  },
  {
    id: "pz-52",
    dayId: 19,
    ...
    category: "JOINs & Subqueries",
    businessScenario:
      "We want to see only premium customers alongside their order details. The current query still lists all value and student customers in the output.",
...
LEFT JOIN orders o ON c.customer_id = o.customer_id AND c.segment = 'Premium';`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The LEFT JOIN ON Filter Trap'.",
      "Connect the primary table with related foreign key tables using INNER JOIN or LEFT JOIN.",
      "Structure your query using clauses similar to: SELECT c.customer_id, c.full_name, o.order_id.",
    ],
...
WHERE c.segment = 'Premium';`,
    ...
    mistakeExplanation:
      "Adding a filter for the left table (customers) in the LEFT JOIN's ON clause only controls which right-table records (orders) get matched. It doesn't filter the left table, so non-premium rows still appear in the output. Use the WHERE clause to filter the left table.",
  },
  {
    id: "pz-53",
    dayId: 13,
...
SELECT office_city FROM departments;`,
  ...
  },
  {
    ...
    category: "Aggregation",
    businessScenario:
      "We want to check if the total company payroll exceeds ₹50 Lakhs. The query executes but returns a single employee name.",
...
HAVING SUM(salary_lpa) > 50;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'HAVING without GROUP BY'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT SUM(salary_lpa) as total_payroll.",
    ],
...
HAVING SUM(salary_lpa) > 50;`,
    ...
    mistakeExplanation:
      "Using HAVING without a GROUP BY aggregates the entire table into a single row. Selecting a non-aggregate field like employee_name returns an arbitrary value from the table. To check the total payroll, select only the aggregate.",
  },
  {
    id: "pz-56",
    dayId: 22,
    ...
    category: "Window Functions",
    businessScenario:
      "Find the previous employee's salary when sorted by salary. The query returns the current row's salary.",
    flawedQuery: `-- Offset of 0 returns the current row
SELECT employee_name, salary_lpa,
  LAG(salary_lpa, 0) OVER (ORDER BY salary_lpa) as prev_salary
FROM employees;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'The Zero-Offset LAG'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT employee_name, salary_lpa,.",
    ],
    solutionQuery: `SELECT employee_name, salary_lpa,
...
FROM employees;`,
    ...
    mistakeExplanation:
      "The LAG(col, offset) function defaults to offset=1. Specifying an offset of 0 queries the current row itself. To retrieve the previous row's value, set the offset to 1.",
  },
  {
    id: "pz-57",
    dayId: 5,
    title: "Case-Insensitive String Matches",
    difficulty: "Easy",
    category: "Syntax & Logic",
    businessScenario:
      "Find all customers living in 'mumbai' (lowercase). The query returns 0 rows even though we have customers in Mumbai.",
    flawedQuery: `-- BINARY forces a case-sensitive comparison in MySQL
SELECT full_name
FROM customers
WHERE BINARY city = 'mumbai';`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'Case-Insensitive String Matches'.",
      "Filter your result set using WHERE conditions to isolate the target criteria.",
      "Structure your query using clauses similar to: SELECT full_name.",
    ],
    solutionQuery: `SELECT full_name
...
WHERE LOWER(city) = 'mumbai';`,
    hint: "MySQL's BINARY operator makes the comparison case-sensitive. Remove that behavior with LOWER().",
    mistakeExplanation:
      "MySQL's default collations are commonly case-insensitive, but BINARY forces an exact case-sensitive comparison. Use LOWER() or a case-insensitive collation when matching user-entered text.",
  },
  {
    id: "pz-58",
    dayId: 21,
    ...
    category: "Window Functions",
    businessScenario:
      "Rank products by price. Tied prices should receive the same rank, but the query assigns different ranks to identical prices.",
...
FROM products;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'ROW_NUMBER vs DENSE_RANK Ties'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
...
FROM products;`,
    ...
    mistakeExplanation:
      "ROW_NUMBER() is strict and always increments sequentially, ignoring ties. To assign identical ranks to identical values without skipping numbers, DENSE_RANK() should be used.",
  },
  {
    id: "pz-59",
    dayId: 8,
    ...
    category: "Aggregation",
    businessScenario:
      "Count how many orders received a discount. The query returns all orders even if they had a discount of 0.0.",
    flawedQuery: `-- This counts the row if discount_amount is 0.0
SELECT COUNT(discount_amount) as discounted_orders
FROM orders;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'COUNT Null Value Trap'.",
      "Apply standard filtering and analytical clauses to isolate the exact target dataset.",
      "Structure your query using clauses similar to: SELECT COUNT(CASE WHEN discount_amount > 0 THEN 1 END) as discounted_orders.",
    ],
...
FROM orders;`,
    ...
    mistakeExplanation:
      "COUNT(column) counts every row where the column is not NULL. Because 0.0 is a non-null number, it gets counted. To count only orders with actual discounts, use a CASE WHEN clause to filter values greater than 0.",
  },
  {
    id: "pz-60",
    dayId: 10,
    ...
    category: "Syntax & Logic",
    businessScenario:
      "Count the number of signups per year. The query displays identical years in separate rows.",
...
GROUP BY signup_date;`,
    hints: [
      "Focus on identifying the core table(s) and columns required for 'Grouping By Unaggregated Field'.",
      "Group your output rows using GROUP BY and apply aggregate functions like COUNT(), SUM(), or AVG().",
      "Structure your query using clauses similar to: SELECT SUBSTR(signup_date, 1, 4) as signup_year, COUNT(*) as count.",
    ],
